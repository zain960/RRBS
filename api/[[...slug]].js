 ...
 *
 * An optional catch-all (`[[...slug]]`) so every `/api/*` request lands here
 * with `req.url` still holding the original path — which is what the Express
 * router expects, since routes/index.js is mounted at `/api`. A plain
 * `api/index.js` plus a rewrite would work too, but rewrites rewrite the path,
 * and then every route would need re-mounting.
 *
